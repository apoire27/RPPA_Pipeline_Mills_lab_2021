...
#Data Read in 
```{r,message=False}
data<-read.csv("./Imputs/SMMART_EBN_RBN_5.csv",header=FALSE)
...
Heatmap(data_for_heatmap, 
        ...
        show_row_names = TRUE,
        name="x",
        column_title = "Patients ___ and ___ most variable")
dev.off()
...
<!-- ```{r} -->
<!-- full_data<-full_data %>%mutate(ID = if_else(rn == b1,"b1", ifelse(rn==b2,"b2","SMMART"))) -->
...
<!-- row_ha = HeatmapAnnotation(sampleID = Full_SMMART$ID,col = list(bar = c("b1" = "red", "b2" = "green", "SMMART" = "Black"))) -->
<!-- ``` -->
<!-- Lets sepera -->
<!-- ```{r} -->
