...
```{r}
data<-read.csv("./Imputs/Bx2_NotNorm.csv",header=FALSE)
head(data)
```
...
#Remove the tresh
#NOTE
#THIS GETS RID OF THE LAST 4 OF ALL COLUMNS 
data2$Protein<-str_sub(data2$Protein,1,nchar(data2$Protein)-4)

data2$Protein<-gsub("-", "", data2$Protein)
```
