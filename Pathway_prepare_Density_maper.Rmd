...
data<-read.csv("Outputs/SMMART_EBN_RBN_5_pathway_score_data.csv",header=TRUE)
...
TCGA_histograms <- ggplot(TCGA_long_data, aes(value,group=Subtype_mRNA,fill=Subtype_mRNA)) +
  ...
  ggtitle("Density Maps for TCGA Samples Patient _______ Highlighted")+
  xlab('Value')+ylab('density') + 
  facet_wrap(~Pathway)+  
  geom_vline(data =line_data, aes(xintercept = value,color=Sample),size = 3, alpha=.4)
...
SMART_histograms <- ggplot(patients_graph_data, aes(value)) +
  ...
  theme_minimal()+
  facet_wrap(~ Pathway)+
  ggtitle("Density Maps for SMART Samples Patient ______ Highlighted")+
  geom_vline(data =line_data, aes(xintercept = value,color=Sample), size = 3)
